// Archivo: src/modules/TabsTabs/MenuTab.jsx
...
export const MenuTab = ({ sucursalId }) => {
  ...
  const handleCancelProdClick = () => {
    const tieneDatos =
      prodFormData.nombre ||
      prodFormData.precio_venta > 0 ||
      prodFormData.grupos_vinculados.length > 0;
    if (tieneDatos) {
      Swal.fire({
  ...
  const productosFiltrados = productos.filter((p) => {
    if (!filtroProductos) return true;
    ...
    const matchCategoria = nombreCategoria.toLowerCase().includes(texto);
    return matchNombre || matchCategoria;
  });

  const gruposFiltrados = gruposMaestros.filter((g) => {
    if (!filtroGrupos) return true;
    return g.nombre?.toLowerCase().includes(filtroGrupos.toLowerCase());
  });
    ...
    <div className={s.tabWrapper}>
      ...
      {activeSubTab === "productos" && (
        <div className={mostrarFormularioProd ? s.splitLayout : s.fullLayout}>
          <aside
            className={`${s.adminCard} ${!mostrarFormularioProd ? s.hidden : ""}`}
          >
            <h3 className={s.cardTitle}>
              {editProdId ? "Ajustar Producto" : "Nuevo Producto"}
            </h3>
            ...
            <form onSubmit={handleSubmitProducto} className={s.formColumn}>
              <div className={s.formGrid}>
                <div className={s.formGroup}>
                  ...
                  <SearchableSelect
                    ...
                    formatLabel={(opt) =>
                      `${opt.nombre} ($${(opt.costo_final || 0).toFixed(2)})`
                    }
                    disabled={noTienePermisoProd}
                    onChange={(val) => {
                      const rec = recetasCosteadas.find(
                        (r) => r.nombre === val,
                      );
                      setProdFormData((prev) => ({
                        ...
                        precio_venta:
                          prev.precio_venta ||
                          (rec && rec.precio_venta ? rec.precio_venta : ""),
                      }));
                    }}
                  />
                </div>
              ...
              <div className={s.formGrid}>
                ...
                <div className={s.formGroup}>
                  ...
                  <div
                    className={s.unitDisplayBox}
                    style={{
                      color: getMarginColor(prodFormData.margen_en_vivo),
                      fontWeight: "700",
                    }}
                  >
              ...
              <div className={s.textMuted}>
                ...
                <SearchableSelect
                  ...
                  disabled={noTienePermisoProd}
                  onChange={(val) =>
                    setProdFormData({ ...prodFormData, categoria: val })
                  }
                />
              </div>
              ...
              <tbody>
                {productosFiltrados.length === 0 ? (
                  ...
                  productosFiltrados.map((p) => {
                    ...
                    const margenBase =
                      netoBase > 0
                        ? (((netoBase - costoBase) / netoBase) * 100).toFixed(1)
                        : 0;
                    const nombreCategoria =
                    ...
                    return (
                      <tr key={p.id}>
                        ...
                        <td className={s.td}>
                          <span className={s.textMuted}>{nombreCategoria}</span>
                        </td>
                        ...
                        <td className={s.td}>
                          ...
                          <div
                            ...
                            style={{
                              fontWeight: "700",
                              color: getMarginColor(margenBase),
                            }}
                          >
                          ...
                          <div className={s.actionsWrapper}>
                            ...
                            {puedeBorrar && (
                              <button
                                ...
                                onClick={() =>
                                  confirmDeleteProducto(p.id, p.nombre)
                                }
                              >
                                ❌
                              </button>
      ...
      {activeSubTab === "grupos" && (
        <div className={mostrarFormularioGrupo ? s.splitLayout : s.fullLayout}>
          <aside
            className={`${s.adminCard} ${!mostrarFormularioGrupo ? s.hidden : ""}`}
          >
            <h3 className={s.cardTitle}>
            ...
            <form onSubmit={handleSubmitGrupo} className={s.formColumn}>
              ...
              <div className={s.stack} style={{ gap: "15px" }}>
                ...
                {grupoFormData.opciones.map((opcion, idx) => {
                  const subData = subrecetasDisponibles.find(
                    (s) => s.nombre === opcion.subreceta_id,
                  );
                  ...
                  return (
                    <div
                      key={idx}
                      className={`${s.itemCardRelative} ${s.bgWhite}`}
                    >
                      {!noTienePermisoGrupo && (
                        <button
                          type="button"
                          className={`${s.btnSecondary} ${s.btnRemoveCircle} ${s.btnSmall}`}
                      ...
                      <div className={s.formGroup}>
                        <SearchableSelect
                          ...
                          formatLabel={(opt) =>
                            `${opt.nombre} - $${(opt.costo_final || 0).toFixed(2)} por ${opt.unidad_abreviatura || "unidad"}`
                          }
                          disabled={noTienePermisoGrupo}
                          onChange={(val) =>
                            updateOpcion(idx, "subreceta_id", val)
                          }
                        />
                      </div>
                      ...
                      <div className={s.formGrid}>
                        <div className={s.formGroup}>
                          <label className={`${s.label} ${s.labelTiny}`}>
                            CANTIDAD ({unidadAbrev})
                          </label>
                          <input
                            type="number"
                            step="0.001"
                            ...
                            onChange={(e) =>
                              updateOpcion(idx, "cantidad", e.target.value)
                            }
                            required
                            disabled={noTienePermisoGrupo}
                          />
                        ...
                        <div className={s.formGroup}>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              updateOpcion(idx, "precio_venta", e.target.value)
                            }
                            required
                            disabled={noTienePermisoGrupo}
                          />
                        ...
                        <div className={s.formGroup}>
                          ...
                          <div
                            className={s.unitDisplayBox}
                            style={{
                              color: getMarginColor(opcion.margen),
                              fontWeight: "700",
                            }}
                          >
                            {opcion.margen}%
                ...
                {!noTienePermisoGrupo && (
                  ...
                  >
                    {loading
                      ? "..."
                      : editGrupoId
                        ? "ACTUALIZAR GRUPO MAESTRO"
                        : "GUARDAR GRUPO MAESTRO"}
                  </button>
                )}
                {editGrupoId && (
                  <button
                    type="button"
                    className={`${s.btn} ${s.btnDark} ${s.btnFull}`}
                    onClick={handleCancelGrupoClick}
                  >
                    {puedeEditar ? "CANCELAR EDICIÓN" : "CERRAR DETALLE"}
                  </button>
                )}
                    ...
                    <tr key={g.id}>
                      <td className={s.td}>
                        ...
                        <small className={s.textMuted}>
                          {g.min_seleccion > 0 ? "Obligatorio" : "Opcional"} •
                          Máx: {g.max_seleccion}
                        </small>
                      </td>
                      <td className={s.td}>
                        <div className={s.formColumnGap5}>
                          {(g.opciones_modificadores || []).map((op, i) => {
                            const vNeta =
                              parseFloat(op.precio_venta) / IVA_FACTOR;
                            const subData = subrecetasDisponibles.find(
                              (s) => s.nombre === op.subreceta_id,
                            );
                            const unidadAbrev =
                              subData?.unidad_abreviatura || "Pz";
                            const cSub =
                              (subData ? subData.costo_final : 0) *
                              (op.cantidad || 1);
                            const mSub =
                              vNeta > 0
                                ? (((vNeta - cSub) / vNeta) * 100).toFixed(1)
                                : 0;

                            return (
                              <div key={i} className={s.labelSmall}>
                                •{" "}
                                <span className={s.fontWeight700}>
                                  {op.cantidad || 1} {unidadAbrev}
                                </span>{" "}
                                de {op.subreceta_id} (+$
                                {parseFloat(op.precio_venta).toFixed(2)})
                                <span
                                  className={s.labelSmall}
                                  style={{
                                    fontWeight: "700",
                                    color: getMarginColor(mSub),
                                    marginLeft: "5px",
                                  }}
                                >
                                  {mSub}% Margen
                      ...
                      <td className={`${s.td} ${s.tdRight}`}>
                        <div className={s.actionsWrapper}>
                          ...
                          {puedeBorrar && (
                            <button
                              className={`${s.btn} ${s.btnOutlineDanger} ${s.btnSmall}`}
                              onClick={() => confirmDeleteGrupo(g.id, g.nombre)}
                            >
                              ❌
                            </button>
                          )}
