...
// CORRECCIÓN: Definido fuera del componente — no se recrea en cada render
...
export default function Trabajadores({ onVolver }) {
  ...
  const abrirEditar = (traba) => {
    setFormData({
      ...
      password:        '',   // Nunca se pre-llena — solo se envía si el admin escribe uno nuevo
      puesto:          traba.puesto          || '',
      rol_id:          traba.rol_id          || '',
        ...
        <section className={styles.card} style={{ animation: 'slideUp 0.3s ease', display: 'flex', flexDirection: 'column', gap: 'var(--space-md)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '15px' }}>
            ...
            <div>
              ...
              <input type="text" name="nombre_completo" value={formData.nombre_completo} onChange={handleInputChange} className={styles.inputEditorial} placeholder="Ej: Juan Pérez Maldonado" />
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              <div>
                <label className={styles.labelTop}>USUARIO (LOGIN) *</label>
                <input type="text" name="usuario" value={formData.usuario} onChange={handleInputChange} className={styles.inputEditorial} placeholder="perez.juan" />
              </div>
              <div>
                <label className={styles.labelTop}>CONTRASEÑA {formData.id && '(Opcional)'}</label>
                <input type="password" name="password" value={formData.password} onChange={handleInputChange} className={styles.inputEditorial} placeholder="••••••••" />
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              <div>
                ...
                <input type="text" name="puesto" value={formData.puesto} onChange={handleInputChange} className={styles.inputEditorial} placeholder="Ej: Cocinero A" />
              </div>
              <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              <div>
                ...
                <input type="date" name="fecha_ingreso" value={formData.fecha_ingreso} onChange={handleInputChange} className={styles.inputEditorial} />
              </div>
              <div>
                <label className={styles.labelTop}>TURNO ASIGNADO *</label>
                <select name="turno" value={formData.turno} onChange={handleInputChange} className={styles.selectEditorial}>
                ...
                >
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '2px', minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.7rem', color: 'var(--text-muted)' }}>
                      ...
                      <span style={{
                        fontWeight: 'bold',
                        color: traba.turno === 'AM' ? COLOR_TURNO_AM
                             : traba.turno === 'PM' ? COLOR_TURNO_PM
                             : 'inherit'
                      }}>
