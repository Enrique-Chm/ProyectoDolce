  ...
  const renderCabecera = () => {
    if (viewMode === 'insumo') {
      return (
        <tr>
          ...
          <th className={s.th}>REQUERIDO MAÑANA</th>
          <th className={s.th}>VENTA PROYECTADA</th>
          <th className={s.th}>STOCK ACTUAL</th>
          <th className={s.th}>SUGERIDO (FALTANTE)</th>
    ...
    return (
      <tr>
        ...
        <th className={s.th} style={{ textAlign: 'center' }}>PROMEDIO ({diaProyectado.toUpperCase()})</th>
        <th className={s.th} style={{ textAlign: 'right' }}>PRÓXIMA VENTA</th>
      </tr>
    );
  };
        ...
        <tr key={item.insumo_id}>
          ...
          <td className={s.td}>
            <span style={{ fontWeight: '800', color: 'var(--color-primary)' }}>
              {item.consumo_diario_real}
            </span>
            <small className={s.textMuted}> {item.unidad_medida}</small>
            <div className={s.textMuted} style={{ fontSize: '10px' }}>Necesario para {diaProyectado}</div>
          </td>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                {tempPolitica.metodo === 'dinamico' ? (
                  <div style={{ display: 'flex', gap: '4px' }}>
                    <input type="number" title="Días Cobertura" placeholder="Cob." className={s.inputField} style={{ width: '55px', padding: '4px' }} value={tempPolitica.cobertura} onChange={e => setTempPolitica({...tempPolitica, cobertura: e.target.value})} />
                    <input type="number" title="Días Seguridad" placeholder="Seg." className={s.inputField} style={{ width: '55px', padding: '4px' }} value={tempPolitica.seguridad} onChange={e => setTempPolitica({...tempPolitica, seguridad: e.target.value})} />
                  </div>
                ) : (
                  <div style={{ display: 'flex', gap: '4px' }}>
                    <input type="number" title="Mínimo" placeholder="Mín." className={s.inputField} style={{ width: '55px', padding: '4px' }} value={tempPolitica.minimo} onChange={e => setTempPolitica({...tempPolitica, minimo: e.target.value})} />
                    <input type="number" title="Máximo" placeholder="Máx." className={s.inputField} style={{ width: '55px', padding: '4px' }} value={tempPolitica.maximo} onChange={e => setTempPolitica({...tempPolitica, maximo: e.target.value})} />
                  </div>
                )}
              </div>
            ) : (
              <div>
                <span className={s.badge} style={{ marginBottom: '4px', display: 'inline-block', fontSize: '10px', backgroundColor: '#e3f2fd', color: '#1976d2' }}>
                  {item.metodo_compra === 'estatico' ? 'STOCK FIJO' : 'DEMANDA JIT'}
                </span>
                <div style={{ fontSize: '11px', fontWeight: '600' }}>
                  {item.metodo_compra === 'estatico' 
                    ? `Mín: ${item.stock_minimo} | Máx: ${item.stock_maximo}`
                    : `Analizando pedidos de ${diaProyectado}`
                  }
                </div>
              </div>
            )}
          ...
          <td className={s.td}>
            <span style={{ 
              fontWeight: '700',
              color: parseFloat(item.stock_fisico_hoy) < parseFloat(item.consumo_diario_real) ? 'var(--color-danger)' : 'inherit'
            }}>
          ...
          <td className={s.td}>
            <div className={s.productTitle} style={{ color: item.cantidad_sugerida > 0 ? 'var(--color-success)' : '#888' }}>
              {item.cantidad_sugerida > 0 ? `+${item.cantidad_sugerida}` : '0'} <small>{item.unidad_medida}</small>
            </div>
            ...
            {puedeEditarInventario && (
              isEditing ? (
                <div style={{ display: 'flex', gap: '5px', justifyContent: 'center' }}>
                  ...
                  <button className={`${s.btn} ${s.btnDanger} ${s.btnSmall}`} onClick={cancelarEdicion}>❌</button>
                </div>
              ) : (
                <button className={`${s.btn} ${s.btnEditar} ${s.btnSmall}`} onClick={() => iniciarEdicion(item)}>📝</button>
              )
            )}
          </td>
        </tr>
  ...
  const renderFilasProductos = () => {
    return dataFiltrada.map((item, i) => (
      <tr key={i}>
        ...
        <td className={s.td} style={{ textAlign: 'center' }}>{item.promedio_diario} unidades</td>
        <td className={s.td} style={{ textAlign: 'right' }}>
          <span className={s.badgeSuccess} style={{ fontSize: '13px' }}>
            {item.prediccion_manana} pedidos proyectados
          </span>
        </td>
      </tr>
    ));
  ...
  return (
    <>
      <section className={s.pageHeader} style={{ marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h2 style={{ fontSize: '1.2rem', margin: 0 }}>Estrategia de Abastecimiento</h2>
          <p className={s.textMuted} style={{ fontSize: '12px' }}>Cálculo automático basado en la demanda de mañana y tus recetas.</p>
        </div>
        <div className={s.adminCard} style={{ padding: '10px 20px', borderLeft: '4px solid var(--color-primary)', margin: 0 }}>
          <span style={{ fontSize: '11px', fontWeight: '700', color: '#888', display: 'block' }}>INVERSIÓN ESTIMADA PARA MAÑANA</span>
      ...
      <div className={`${s.adminCard} ${s.tableContainer}`}>
        <div style={{ padding: "16px", display: "flex", gap: "12px", background: '#fcfcfc', borderBottom: '1px solid #eee' }}>
          <input 
            type="text" 
            className={s.inputField} 
        ...
        <table className={s.table}>
          ...
          <tbody>
            {dataFiltrada.length > 0 ? (
              ...
              <tr>
                <td colSpan={viewMode === 'insumo' ? "6" : "3"} className={s.td} style={{ textAlign: 'center', padding: '40px', color: '#888' }}>
