...
export const CajeroTab = ({ sucursalId, usuarioId }) => {
  ...
  // PANTALLA DE BLOQUEO: APERTURA (Si no hay turno activo)
  if (!turnoActivo) {
    return (
      <div className={s.modalOverlay}>
        <div className={s.modalCierre} style={{ textAlign: 'center' }}>
          <h2 className={s.modalTotalValue} style={{fontSize: '2rem'}}>Apertura de Turno</h2>
          ...
          <div style={{ margin: '30px 0' }}>
            ...
            <input 
              ...
              value={montoInicial} onChange={(e) => setMontoInicial(e.target.value)} autoFocus
            />
          </div>
          <button 
  ...
  return (
    <div className={s.posContainer}>
        ...
        <>
          ...
          <div className={s.productGrid}>
            {cuentasPorCobrar.map(v => (
              <div key={v.id} className={s.mesaCard} style={{ borderLeft: '8px solid var(--warning)' }} onClick={() => abrirCobro(v)}>
      ...
      {activeSubTab === 'movimientos' && (
        <div className={s.modalCierre} style={{ maxWidth: '600px', margin: '0 auto' }}>
          ...
          <div style={{ marginTop: '20px' }}>
            ...
            <button className={s.btnOrder} onClick={handleMovimientoManual}>REGISTRAR EN CAJA</button>
          </div>
        </div>
      )}
      ...
      {activeSubTab === 'corte' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
          <div style={{ background: 'white', padding: '30px', borderRadius: '32px', border: '1px solid var(--border-light)' }}>
            ...
            <div style={{ margin: '20px 0', display: 'grid', gap: '15px' }}>
              ...
              <div className={s.changeRow} style={{borderTop:'2px solid var(--border-light)', paddingTop:'15px'}}>
                <span style={{fontWeight:800}}>ESPERADO EN EFECTIVO:</span>
                <span className={s.modalTotalValue} style={{fontSize:'1.8rem'}}>${(parseFloat(turnoActivo.monto_apertura) + resumenCaja.efectivo).toFixed(2)}</span>
              </div>
            </div>
          ...
          <div style={{ background: 'white', padding: '30px', borderRadius: '32px', border: '1px solid var(--border-light)' }}>
            ...
            <div style={{ maxHeight: '400px', overflowY: 'auto', marginTop: '15px' }}>
              {historialHoy.map(h => (
                <div key={h.id} className={s.changeRow}>
                  <div><span style={{fontWeight:800}}>Mesa {h.mesa}</span><br/><small className={s.textMuted}>{h.metodo_pago.toUpperCase()}</small></div>
                  <strong style={{fontSize:'1.1rem'}}>${h.total}</strong>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* MODAL UNIVERSAL DE COBRO */}
      {ventaSeleccionada && (
        <div className={s.modalOverlay}>
          <div className={s.modalCierre}>
            <button className={s.btnCloseModal} onClick={() => setVentaSeleccionada(null)}>✕</button>
            <h2 className={s.modalTotalValue} style={{fontSize: '2rem', marginBottom:'20px'}}>Mesa {ventaSeleccionada.mesa}</h2>
            ...
            <label className={s.formLabel}>Propina ($)</label>
            ...
            {metodoPago === 'efectivo' && (
              <div className={s.cashBox}>
                <label className={s.formLabel}>Pagó con:</label>
                <input 
                  type="number" className={`${s.input} ${faltaDinero > 0 ? s.inputError : s.inputSuccess}`} 
                  value={pagadoCon} onChange={e => setPagadoCon(e.target.value)} autoFocus
                />
                <div className={s.changeRow}>
                  {faltaDinero > 0 ? (
                    <><span className={s.warningText} style={{color:'var(--danger)', fontWeight:800}}>⚠️ FALTA:</span><span style={{color:'var(--danger)', fontWeight:900, fontSize:'1.5rem'}}>-${faltaDinero.toFixed(2)}</span></>
                  ) : (
                    <><span style={{color:'var(--success)', fontWeight:800}}>CAMBIO:</span><span style={{color:'var(--success)', fontWeight:900, fontSize:'1.5rem'}}>${cambio.toFixed(2)}</span></>
                  )}
                </div>
              </div>
            )}

            <div className={s.paymentGrid} style={{marginTop:'20px'}}>
              <button className={s.btnCancel} onClick={() => setVentaSeleccionada(null)}>VOLVER</button>
              <button 
                className={s.btnOrder} 
