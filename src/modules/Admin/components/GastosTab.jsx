            ...
            <aside className={s.adminCard}>
              ...
              <form onSubmit={g.handleSaveGasto} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                  ...
                  {g.editGastoId && (
                    <button type="button" className={`${s.btn} ${s.btnOutlineDanger}`} onClick={g.resetGastoForm}>
                      CANCELAR
                    </button>
                  )}
                </div>
          ...
          <div className={`${s.adminCard} ${s.tableContainer}`}>
            {g.loading ? (
              ...
              <table className={s.table} style={{ minWidth: '700px' }}>
                ...
                <tbody>
                  {g.gastos.length === 0 ? (
                    <tr><td colSpan="5" className={s.emptyState}>No hay gastos registrados.</td></tr>
                  ) : g.gastos.map(item => (
                    <tr key={item.id}>
                      <td className={s.td} style={{ fontSize: '13px' }}>{item.fecha}</td>
                      <td className={s.td}>
                        <div style={{ fontWeight: '600', color: 'var(--color-text-main)', fontSize: '14px' }}>{item.descripcion}</div>
                      ...
                      <td className={s.td} style={{ fontWeight: '600', color: 'var(--color-danger)' }}>
                        ${parseFloat(item.monto).toFixed(2)}
                      </td>
                      <td className={s.td} style={{ textAlign: 'right' }}>
                        <div style={{ display: 'flex', gap: '5px', justifyContent: 'flex-end' }}>
                          {g.puedeEditarGastos && (
                            <button className={`${s.btn} ${s.btnOutlineEditar} ${s.btnEditar}`} onClick={() => { g.setEditGastoId(item.id); g.setGastoFormData(item); }}>
                              📝
                            </button>
                          )}
                          {g.puedeBorrarGastos && (
                            <button className={`${s.btn} ${s.btnOutlineDanger} ${s.btnSmall}`} onClick={() => g.handleDeleteGasto(item.id)}>
                              Borrar
                            </button>
                          )}
                        </div>
            ...
            <aside className={s.adminCard}>
              ...
              <form onSubmit={g.handleSaveCategoria} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                  ...
                  {g.editCatId && (
                    <button type="button" className={`${s.btn} ${s.btnOutlineDanger}`} onClick={g.resetCatForm}>
                      CANCELAR
                    </button>
                  )}
                </div>
            ...
            <table className={s.table} style={{ minWidth: '400px' }}>
               ...
               <tbody>
                 {g.categorias.length === 0 ? (
                    <tr><td colSpan="3" className={s.emptyState}>No hay categorías registradas.</td></tr>
                 ) : g.categorias.map(c => (
                   <tr key={c.id}>
                     <td className={s.td} style={{ fontWeight: '600', color: 'var(--color-text-main)' }}>{c.nombre}</td>
                     <td className={s.td} style={{ fontSize: '13px', color: 'var(--color-text-muted)' }}>{c.descripcion || '-'}</td>
                     <td className={s.td} style={{ textAlign: 'right' }}>
                       <div style={{ display: 'flex', gap: '5px', justifyContent: 'flex-end' }}>
                         {g.puedeEditarGastos && (
                           <button className={`${s.btn} ${s.btnOutlineEditar} ${s.btnEditar}`} onClick={() => { g.setEditCatId(c.id); g.setCatFormData(c); }}>
                             📝
                           </button>
                         )}
                         {g.puedeBorrarGastos && (
                           <button className={`${s.btn} ${s.btnOutlineDanger} ${s.btnSmall}`} onClick={() => g.handleDeleteCategoria(c.id)}>
                             ❌
                           </button>
                         )}
                       </div>
